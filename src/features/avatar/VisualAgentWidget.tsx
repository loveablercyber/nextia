...
interface PublicConfig {
  ...
  config: { model: AvatarModelId; desktop: boolean; tablet: boolean; mobile: boolean; desktopSize: number; tabletSize: number; mobileSize: number; position: 'left' | 'right'; draggable: boolean; minimizable: boolean; hideable: boolean; voiceAutoplay: boolean; volume: number; speechRate: number; idleSeconds: number; agentName: string; welcomeMessage: string; performanceProfile: 'high' | 'balanced' | 'economy' | 'fallback' };
}
...
export default function VisualAgentWidget() {
  ...
  return <aside aria-label={`Assistente ${settings.config.agentName}`} style={{ transform: viewport === 'desktop' ? `translate(${offset.x}px, ${offset.y}px)` : undefined }} className={`fixed bottom-[max(12px,env(safe-area-inset-bottom))] z-[80] ${alignment} ${deviceClass} flex-col items-end gap-2`}>
    {open && <section className="mb-2 flex h-[min(520px,70vh)] w-[min(360px,calc(100vw-24px))] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl" role="dialog" aria-label="Conversa com o assistente">
    ...
    {!minimized && <div data-avatar-state={state} onPointerDown={beginDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} style={{ height: avatarHeight, width: Math.round(avatarHeight * .794), touchAction: viewport === 'desktop' && settings.config.draggable ? 'none' : 'auto' }} className={`relative overflow-hidden rounded-[28px] bg-transparent ${viewport === 'desktop' && settings.config.draggable ? 'cursor-grab active:cursor-grabbing' : ''}`}>
      {settings.live2dEnabled && settings.config.performanceProfile !== 'fallback' && fallback === 0 ? <Suspense fallback={null}><Live2DCanvas modelId={modelId} state={state} onFailure={handleLive2DFailure}/></Suspense> : fallback < 2 ? <img src={model.fallbackImageUrl} alt={model.label} className="h-full w-full object-contain" onError={() => setFallback(2)}/> : <button type="button" onClick={() => setOpen(true)} className="absolute bottom-0 right-0 grid h-14 w-14 place-items-center rounded-full bg-[#6535D9] text-white shadow-xl" aria-label="Abrir assistente"><Bot/></button>}
    </div>}
